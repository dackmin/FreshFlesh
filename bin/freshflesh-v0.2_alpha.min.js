...
FF.Animation=function(a,b,c){FF.EventManager.call(this);var d=this;this.node=a;this.timer=new FF.Timer({timeout:b,interval:c.step||2,autostart:c.autostart});this.timer.addEventListener("start",function(){d.dispatchEvent({type:"start"})});this.timer.addEventListener("tick",function(){d.animate();d.dispatchEvent({type:"step"})});this.timer.addEventListener("timeout",function(){d.dispatchEvent({type:"end"})});this.animateLeft=c.left||null;this.animateRight=c.right||null;this.animateTop=c.top||null;this.animateBottom=
c.bottom||null;this.animateWidth=c.width||null;this.animateHeight=c.height||null;this.animateScale=c.scale||null;this.animateOpacity=c.opacity||null};FF.Animation.prototype.update=function(){this.timer.update()};
FF.Animation.prototype.animate=function(){this.animateLeft&&(this.node.x-=this.animateLeft);this.animateRight&&(this.node.x+=this.animateRight);this.animateTop&&(this.node.y-=this.animateTop);this.animateBottom&&(this.node.y+=this.animateBottom);this.animateWidth&&(this.node.width+=this.animateWidth);this.animateHeight&&(this.node.height+=this.animateHeight);this.animateScale&&0<this.node.scale&&(this.node.scale+=this.animateScale);this.animateOpacity&&0<this.node.opacity&&(this.node.opacity+=this.animateOpacity)};
