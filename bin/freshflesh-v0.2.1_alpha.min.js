...
FF.Animation.prototype.animate=function(a){a=this.timer.time/this.timer.timeout;this.animateLeft&&(this.node.x-=this.animateLeft*a);this.animateRight&&(this.node.x+=this.animateRight*a);this.animateTop&&(this.node.y-=this.animateTop*a);this.animateBottom&&(this.node.y+=this.animateBottom*a);if(this.animateWidth){var b=this.node.width-this.animateWidth;this.node.width+=b*a}this.animateHeight&&(b=this.node.height-this.animateHeight,this.node.height+=b*a);this.animateScale&&(b=this.node.scale-this.animateScale,
this.node.scale+=b*a);this.animateOpacity&&(b=this.node.alpha-this.animateOpacity,this.node.alpha+=b*a)};FF.Animation.prototype.start=function(){this.timer.restart()};FF.EventManager=function(){var a=[];this.addEventListener=function(b,c){void 0===a[b]&&(a[b]=[]);-1===a[b].indexOf(c)&&a[b].push(c)};this.dispatchEvent=function(b){for(var c in a[b.type])a[b.type][c](b)};this.removeEventListener=function(b,c){var d=a[b].indexOf(c);-1!==d&&a[b].splice(d,1)}};
