...
FF.Animation=function(a,b,c){FF.EventManager.call(this);var d=this;this.node=a;this.timer=new FF.Timer({timeout:b,interval:c.step||100,autostart:c.autostart||!1});this.timer.addEventListener("start",function(){d.dispatchEvent({type:"start"})});this.timer.addEventListener("tick",function(a){d.animate(a);d.dispatchEvent({type:"step"})});this.timer.addEventListener("timeout",function(){d.dispatchEvent({type:"end"})});this.animateLeft=c.left||null;this.animateRight=c.right||null;this.animateTop=c.top||
null;this.animateBottom=c.bottom||null;this.animateWidth=c.width||null;this.animateHeight=c.height||null;this.animateScale=c.scale||null;this.animateOpacity=c.opacity||null};FF.Animation.prototype.update=function(){this.timer.update()};
FF.Animation.prototype.animate=function(a){a=this.timer.time/this.timer.timeout;null!==this.animateLeft&&(this.node.x-=this.animateLeft*a);null!==this.animateRight&&(this.node.x+=this.animateRight*a);null!==this.animateTop&&(this.node.y-=this.animateTop*a);null!==this.animateBottom&&(this.node.y+=this.animateBottom*a);if(null!==this.animateWidth){var b=this.animateWidth-this.node.width;this.node.width+=b*a}null!==this.animateHeight&&(b=this.animateHeight-this.node.height,this.node.height+=b*a);null!==
this.animateScale&&(b=this.animateScale-this.node.scale,this.node.scale+=b*a);null!==this.animateOpacity&&(b=this.animateOpacity-this.node.alpha,this.node.alpha+=b*a)};FF.Animation.prototype.start=function(){this.timer.restart()};
FF.EventManager=function(){var a=[];this.addEventListener=function(b,c){void 0===a[b]&&(a[b]=[]);-1===a[b].indexOf(c)&&a[b].push(c)};this.dispatchEvent=function(b){for(var c in a[b.type])a[b.type][c](b)};this.removeEventListener=function(b,c){var d=a[b].indexOf(c);-1!==d&&a[b].splice(d,1)}};
