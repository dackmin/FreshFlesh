...
FF.AssetLoader.prototype.load=function(){var a=this;for(i in this.assetsURL){var b=this.assetsURL[i],c=this.assetsURL[i].split(".").pop().toLowerCase(),c=this.assetsTypes[c];if(!c)throw Error("Filetype of ["+b+"] is not supported");c=new c(b);c.addEventListener("loaded",function(){a.loadedAsset()});c.load()}};
FF.AssetLoader.prototype.loadedAsset=function(){this.loadedCount++;this.dispatchEvent({type:"progress",loaded:this.loadedCount,total:this.assetsURL.length});this.loadedCount==this.assetsURL.length&&this.dispatchEvent({type:"loaded"})};
...
FF.Animation=function(a){FF.EventManager.call(this);this.frames=a.frames||sprite.positions;this.subsets=[];this.frame_duration=a.frame_duration||100;this.index=0;this.loop=1;this.bounce=0;this.frame_direction=1;this.frame_size=null;this.orientation="down";this.on_end=null;this.offset=0;this.sprite_sheet=this.scale_image=null;this.current_tick=(new Date).getTime();this.last_tick=(new Date).getTime();this.sum_tick=0;if(a.subsets)for(subset in this.subsets={},a.subsets)start_stop=a.subsets[subset],this.subsets[subset]=
this.slice(start_stop[0],start_stop[1])};FF.Animation.prototype.subset=function(a){return this.subsets[a]};
FF.Animation.prototype.update=function(){this.current_tick=(new Date).getTime();this.sum_tick+=this.current_tick-this.last_tick;this.last_tick=this.current_tick;this.sum_tick>this.frame_duration&&(this.index+=this.frame_direction,this.sum_tick=0);if(this.index>=this.frames.length||0>this.index)this.bounce?(this.frame_direction=-this.frame_direction,this.index+=2*this.frame_direction):this.loop?this.index=0>this.frame_direction?this.frames.length-1:0:(this.index-=this.frame_direction,this.on_end&&
(this.on_end(),this.on_end=null))};FF.Animation.prototype.slice=function(a,b){var c={};c.frame_duration=this.frame_duration;c.loop=this.loop;c.bounce=this.bounce;c.on_end=this.on_end;c.frame_direction=this.frame_direction;c.frames=this.frames.slice().slice(a,b);return new FF.Animation(c)};
FF.Animation.prototype.merge=function(a){var b={};b.frame_duration=this.frame_duration;b.loop=this.loop;b.bounce=this.bounce;b.on_end=this.on_end;b.frame_direction=this.frame_direction;b.frames=[];for(var c in a)b.frames.push(this.frames[a[c]]);return new FF.Animation(b)};FF.Animation.prototype.next=function(){this.update();return this.frames[this.index]};FF.Animation.prototype.atLastFrame=function(){return this.index==this.frames.length-1};
FF.Animation.prototype.atFirstFrame=function(){return 0==this.index};FF.Animation.prototype.currentFrame=function(){return this.frames[this.index]};FF.EventManager=function(){var a=[];this.addEventListener=function(b,c){void 0===a[b]&&(a[b]=[]);-1===a[b].indexOf(c)&&a[b].push(c)};this.dispatchEvent=function(b){for(var c in a[b.type])a[b.type][c](b)};this.removeEventListener=function(b,c){var d=a[b].indexOf(c);-1!==d&&a[b].splice(d,1)}};
